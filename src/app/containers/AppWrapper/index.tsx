...
import { Link, Avatar, Fa, Button, FaDiv, Div } from 'app/components';
import { List, ListItem, ListItemText, IconButton, Icon } from '@material-ui/core';
...
const sortByVolAsc = (a,b) => { return a.vol - b.vol }
const sortByVolDesc = (a,b) => { return b.vol - a.vol }
...
const arrows = ["Pair","Vol","Price", "24h Chg%"];
  ...
  render(){
    ...
    if(selected > 0){
      ...
      switch(sorter.value){
        ...
        case 1:
          if(sorter.dir){
            rel = c_currency.rel.sort(sortByVolAsc);
          }else{
            rel = c_currency.rel.sort(sortByVolDesc);
          }
        break;
        case 2:
          if(sorter.dir){
            ...
            <Link onClick={()=>{ this.setState({ selected: 0 }) }} clearfix to="/">
              ...
              {selected != 0 &&
               ...
               <FaDiv fs c> 
                ...
                {rel.map( (o, i) =>  (
                  <Link onClick={()=>{ this.setState({ select2: i+1 }) }} ey={i} clearfix to={`/exchange/${c_currency.base}_${o.ticker}`}>
                    ...
                    <Fa fs style={{width: "100px"}}>
                      <FaDiv vcenter>
                        ...
                        <Div className={cx(styles.base)}> / {c_currency.base}</Div>
                      </FaDiv>
                      <FaDiv vcenter>
                        <Div className={cx(styles.vol)}>Vol {o.vol}</Div>
                      </FaDiv>
                    </Fa>
                    <FaDiv fs c style={{width: "86px"}}>
                        ...
                        <Div className={cx(styles.priceusd)}>${o.priceusd}</Div>                      
                    </FaDiv>
                    <FaDiv vcenter fs c style={{width: "70px"}}>
                        <Div className={cx(styles.change,
  						...
  						<FaDiv vcenter className={cx(styles.bottom_bar)}>
  							<Avatar height={40} width={40} src={appStore.user.pp || appStore.user.email}/>
  							<Fa fs className={cx(styles.username)}>{appStore.user.email.split("@")[0]}</Fa>
                <Fa fa>
                  <IconButton color="primary" ><Icon style={{fontSize: 14}} className={cx(classes.icon)}>settings</Icon></IconButton>  
                </Fa>
